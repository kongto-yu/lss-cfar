...
from models.nn import Activation, DropoutNd, Normalization
from models.sequence.backbones.block import SequenceResidualBlock
from models.sequence.kernels.fftconv import FFTConv
...
class MultiHeadEMA(nn.Module):
    ...
    - The only semantic change is removing the final SiLU activation,
      which is handled by the caller module (e.g. models.sequence.mega.MegaBlock).

    It is possible to recreate the functionality of MultiHeadEMA by using other modular blocks,
    in particular the models.sequence.kernels.fftconv.FFTConv block
    together with models.sequence.kernels.kernel.EMAKernel.
    """
