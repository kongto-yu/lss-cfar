...
from models.hippo.hippo import transition
from models.functional.toeplitz import causal_convolution, causal_convolution_inverse, construct_toeplitz
...
def trid_gbt_forward(delta, u, dl, d, du, pl, pr, c):
    """
    Computes (I + delta A) u for a tridiagonal matrix A.

    delta: scalar or tensor, step size
    u: tensor, input vector
    dl: tensor, sub-diagonal of A
    d: tensor, main diagonal of A
    du: tensor, super-diagonal of A
    pl, pr: tensors, scaling factors
    c: tensor, additional constant factor
    """
    n = u.shape[-1]
    ...
    # Initialize the result with (I * u)
    result = u_scaled.clone()
    
    # Apply delta * A * u
    # Note: A is defined by its diagonals dl (sub-diagonal), d (main diagonal), du (super-diagonal)
    
    # Main diagonal contribution
    result += delta * d * u_scaled
    
    # Super-diagonal contribution
    if n > 1:
        result[..., :-1] += delta * du * u_scaled[..., 1:]
    
    # Sub-diagonal contribution
    if n > 1:
        result[..., 1:] += delta * dl * u_scaled[..., :-1]
    
    # Scale result by pl and add the constant term c * u
    result = result * pl + c * u
    
    return result

def trid_gbt_backward(delta, b, dl, d, du, pl, pr, c):
    """
    Solves (I - delta A) u = b for a tridiagonal matrix A.
    
    delta: scalar or tensor, step size
    b: tensor, input vector
    dl: tensor, sub-diagonal of A
    d: tensor, main diagonal of A
    du: tensor, super-diagonal of A
    pl, pr: tensors, scaling factors
    ...
    # Scale b by pr and subtract the constant term c * b
    b_scaled = b * pr - c * b
    
    # Initialize the result u
    u = torch.zeros_like(b_scaled)
    
    # Set up the coefficients for the tridiagonal matrix (I - delta A)
    a = -delta * dl
    c = -delta * du
    d_main = 1.0 - delta * d
    
    # Forward sweep (LU decomposition for tridiagonal matrix)
    cp = torch.zeros_like(c)
    dp = torch.zeros_like(d_main)
    cp[..., 0] = c[..., 0] / d_main[..., 0]
    dp[..., 0] = b_scaled[..., 0] / d_main[..., 0]
    
    for i in range(1, n):
        denom = d_main[..., i] - a[..., i-1] * cp[..., i-1]
        cp[..., i] = c[..., i] / denom
        dp[..., i] = (b_scaled[..., i] - a[..., i-1] * dp[..., i-1]) / denom
...
class AdaptiveTransition(nn.Module):
    ...
    def forward_mult(A, u, delta):
        ...
        # Compute (I + delta A)
        I_plus_delta_A = I + delta_A
        
        # Compute the product (I + delta A) u
        result = np.dot(I_plus_delta_A, u)
